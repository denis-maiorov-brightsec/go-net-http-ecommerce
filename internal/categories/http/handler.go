...
func (h *Handler) Register(mux *http.ServeMux) {
	...
	mux.Handle("POST /v1/categories", apierror.Adapt(h.create))
	mux.Handle("GET /v1/categories/{id}", apierror.Adapt(h.getByID))
	mux.Handle("PATCH /v1/categories/{id}", apierror.Adapt(h.update))
	mux.Handle("DELETE /v1/categories/{id}", apierror.Adapt(h.delete))
}
